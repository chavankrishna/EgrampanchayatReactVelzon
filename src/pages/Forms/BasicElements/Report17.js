...
const Report17 = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post("http://localhost:8080/api/guntonukNamuna25/getAll");
  ...
  const handleDelete = async (selectedRecord) => {
    try {
      const response = await axios.post(`http://localhost:8080/api/guntonukNamuna25/deleteById/${selectedRecord.id}`);

      window.location.href = "/report17";
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <Row>
            <Col lg={12}>
              <Card>
                <Button style={{ width: "13%", justifyContent: "left", marginLeft: "86%", marginTop: "1%" }} color="primary" onClick={() => navigate("/nikita")} className="btn btn-sm">
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                <div className="d-flex gap-2">
                                  <button className="btn btn-sm btn-primary" onClick={() => navigate("/update", { state: data })}>
                                    अद्यतन करा
                                  </button>
                                  <button className="btn btn-sm btn-primary" onClick={() => navigate("/view-details", { state: { id: data.id } })}>
                                    डेटा पहा
                                  </button>
