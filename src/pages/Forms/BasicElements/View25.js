...
const View25 = () => {
      ...
      const fetchRecord = async () => {
        try {
          ...
          const response = await axios.post(
            `http://localhost:8080/api/guntonukNamuna25/getById/${record.id}`,
            {},
            {
              headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token"); // Get the token from local storage
      const response = await axios.post(
        `http://localhost:8080/api/guntonukNamuna25/update/${record.id}`,
        record,
        {},
        {
          headers: {
      ...
      <div className="page-content" style={{ backgroundColor: "#fbf7f4" }}>
        <Container fluid>
          ...
          <Row>
            <Col lg={12}>
              <Card>
                <PreviewCardHeader title="गुंतवणूक वही" buttonLabel="अहवाल" onButtonClick={() => navigate("/नमुना-२५-अहवाल")} />
