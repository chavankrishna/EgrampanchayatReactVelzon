...
const ProfileDropdown = () => {
  ...
  useEffect(() => {
    ...
    if (storedUsername) {
      // Remove the "@gmail.com" part if present
      const formattedUsername = storedUsername.split("@")[0];
      setUserName(formattedUsername);
    } else {
      setUserName("Guest"); // Fallback if no username is stored
    }
  }, []);
        ...
        <DropdownMenu className="dropdown-menu-end">
          ...
          <DropdownItem className="p-0">
            <Link to="/logout" className="dropdown-item">
              <i className="mdi mdi-logout text-muted fs-16 align-middle me-1"></i> <span className="align-middle">Logout</span>
            </Link>
          </DropdownItem>
        </DropdownMenu>
      </Dropdown>
