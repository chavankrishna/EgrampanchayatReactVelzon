...
const ReportPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post("http://localhost:8080/kar-akarani/getall");
  ...
  const handleAddRecord = async () => {
    try {
      const response = await axios.post("http://localhost:8080/kar-akarani/create", newRecord);
      if (response.data) {
        setDataList([...dataList, newRecord]); // Add the new record to the table
        setModalList(false); // Close the modal
        navigate("/Namuna8"); // Navigate to the '/nikita' route
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("तुम्हाला खात्री आहे का की तुम्हाला हा डेटा काढून टाकायचा आहे?")) {
      try {
        ...
        const response = await axios.post(`http://localhost:8080/kar-akarani/delete_by_id/${id}`);
        ...
        <Container fluid>
          <Row>
            <Col lg={12}>
              <Card>
                {/* <CardHeader>
                                    <h4 className="card-title mb-0">कर आकारणी नोंदवही </h4>
                                </CardHeader> */}
                <PreviewCardHeader title="कर आकारणी नोंदवही" buttonLabel="नवीन माहिती प्रविष्ट करा " onButtonClick={() => navigate("/नमुना-८")} />
                <CardBody>
                              ...
                              <tr key={index}>
                                ...
                                <td>
                                  <div className="d-flex gap-2">
                                    <button className="btn btn-sm btn-success edit-item-btn" onClick={() => navigate("/नमुना-८-अपडेट", { state: data })}>
                                      एडिट करा
                                    </button>
                                ...
                                <td>{data.rastyacheNaav}</td>
                                <td>{data.malmattaKramank}</td>
                                <td>{data.malmattecheWarnan}</td>
                                <td>{data.malkacheNaav}</td>
                                <td>{data.bhogvataKarnaracheNaav}</td>
                                <td>{data.kshetraphal}</td>
                                <td>{data.warshikBhadeKinvaKimmatdar}</td>
                                <td>{data.karYadiGharpattiKar}</td>
                                <td>{data.karYadiWeejKar}</td>
                                <td>{data.karYadiArogyaKar}</td>
                                <td>{data.karYadiPanipattiKar}</td>
                                <td>{data.karYadiEkun}</td>
                                <td>{data.nakkalFee}</td>
                                <td>{data.rujuwatFee}</td>
                                <td>{data.kagadFee}</td>
